...
def test_constructed_account_correct_username(db_session):
    ...
    account = Account(username='user', email='user@user.user', password='1234')
...
def test_create_account_without_username(db_session):
    ...
    with pytest.raises(Exception):
        return Account(email='user@user.user', password='1234')
...
def test_duplicate(db_session):
    ...
    from ..models import Account
    account = Account(username='user', email='user@user.user', password='1234')
    account_two = Account(username='user', email='user@user.user', password='1234')
    db_session.add(account)
    db_session.commit()
